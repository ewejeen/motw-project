						...
						<tr class="paging">
							<td colspan="5" align="center">
								<c:if test="${pagingList.total==0 }">
									<tr>
										<td colspan="5" class="nopost one">No results.</td>
									</tr>
								</c:if>
								
								<c:if test="${pagingList.pNum>pagingList.block }">
									<a href="cSearch.bo?crit=${crit }&search=${search }&pNum=1">◀◀　</a>
									<a href="cSearch.bo?crit=${crit }&search=${search }&pNum=${pagingList.startPage-1}">◀</a>								
								</c:if>
								
								<c:forEach var="i" begin="${pagingList.startPage }" end="${pagingList.endPage }">
									<c:if test="${i==pagingList.pNum }">
										<b><a href="cSearch.bo?crit=${crit }&search=${search }&pNum=${i }">${i }</a></b>									
									</c:if>
									<c:if test="${i!=pagingList.pNum }">
										<a href="cSearch.bo?crit=${crit }&search=${search }&pNum=${i }">${i }</a>																			
									</c:if>
								</c:forEach>
								
								<c:if test="${pagingList.endPage < pagingList.totalPage }">
									<a href="cSearch.bo?crit=${crit }&search=${search }&pNum=${pagingList.endPage+1}">▶　</a>
									<a href="cSearch.bo?crit=${crit }&search=${search }&pNum=${pagingList.totalPage}">▶▶</a>								
								</c:if>
